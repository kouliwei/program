...
extern void Set_PID(void)
{
	...
	KI = (float)Real_PcBuff[3]/100;		//积分系数0-2.55
...
static void XiaYang_Control(void)    //既前进又下沉又保证yaw角稳定
{
	...
	u = Yaw_Control(Ref_Yaw);
	if (Forcex < 10)
	{
		Temp1[0] = PWM_STOP + 20 * 15;
		Temp1[1] = PWM_STOP + 20 * (-15);
		Temp1[2] = PWM_STOP + 20 * 19;
		Temp1[3] = PWM_STOP + 20 * (-19);
	}
	else if (Forcex < 20)
	{
		Temp1[0] = PWM_STOP + 20 * 20;
		Temp1[1] = PWM_STOP + 20 * (-20);
		Temp1[2] = PWM_STOP + 20 * 26;
		Temp1[3] = PWM_STOP + 20 * (-26);
	}
	else
	{
		Temp1[0] = PWM_STOP + 20 * 25;
		Temp1[1] = PWM_STOP + 20 * (-25);
		Temp1[2] = PWM_STOP + 20 * 32;
		Temp1[3] = PWM_STOP + 20 * (-32);
	}
	Temp1[3] = Temp1[3] - u;
	Temp1[2] = Temp1[2] + u;
	Temp1[1] = Temp1[1] + u;
	...
	EPwm1Regs.CMPA.half.CMPA = Temp1[0];
	EPwm2Regs.CMPA.half.CMPA = Temp1[1];
	EPwm3Regs.CMPA.half.CMPA = Temp1[2];
	EPwm4Regs.CMPA.half.CMPA = Temp1[3];
...
static int Yaw_Control(float Ref_Yaw)
{
	...
	error = error/(float)18;  //归一化  变成-10  ~  10的范围
	//积分限幅

	if(error > -2.5 && error < 2.5)  //对应-45度 ~45度  45/18=2.5
	{
		error_sum += error;
	}
	...
	if(gl_TempFlag[1] == 1)  //定时完成了,累加置0，为下一次的积分做准备。
	{
		error_sum = 0;
	}
